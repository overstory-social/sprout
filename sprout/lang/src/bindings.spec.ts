...
import { locationOf, SourceFile, type Span } from './source.js';
import { BOOLEAN, integer, OBJECT, STRING } from './types.js';

// --- the world these specs are written about ------------------------------
//
// The printer's shop, the same one the other suites use. Kinds have no
// syntax until B12, so a `KindRef` is built here by hand — which is the
...
/** A property, declared the way a kind would and resolved the way B06 does. */
...
describe('a binding type is not a property type', () => {
  it('excludes the object type from the value arm, because no value is one', () => {
    expect(asHeldType(BOOLEAN)).toEqual(BOOLEAN);
    expect(asHeldType(integer(0, 9))).toEqual(integer(0, 9));
    expect(asHeldType(OBJECT)).toBeNull();
  });
  ...
  it('says what it is, for every arm there is', () => {
    const every: [BindingType, string][] = [
      ...
      [valueOf(asHeldType(KNOWS.type)!), '[Topic]'],
