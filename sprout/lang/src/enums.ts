// Enums, and the symbols that belong to them (B05; the spec's
// Properties › Enums).
//
...
// whole point of the type, and it is the defect the reviews found in the
// previous language, where a symbol evaluated to its own bare name.
//
...
// expressions. Checking a symbol where it is USED is B09's, and calls
// `checkOption` below; the warning for a world declaration shadowing a
// library one is B21's.
 ...
 * assigned to. The one check an enum exists for, called by B09 wherever
 * a symbol meets a typed operand.
 */
