// Typed bindings, exactly as the table (B08; the spec's Properties ›
...
// is three arms rather than a `SproutType`, and the object arm carries
// the kind when the compiler knows it and null when it does not — which
// is the whole of what the object type means.
//
// What a KIND is belongs to B12 and B19. What typing needs of one is
...
//
// Checking an expression against these bindings is B09's; `let` is
// B11's, and calls `letBinding` below; verbs and their roles are B23's,
...
import type { DeclaredMessage } from './messages.js';
import type { ResolvedProperty } from './properties.js';
import type { Span } from './source.js';
import { integer, showType, type SproutType } from './types.js';

// --- kinds, as typing needs them -----------------------------------------

/**
 * A kind, reduced to what typing asks of it. B12 declares kinds and B19
 * composes them; both produce something that satisfies this, and nothing
 * here needs to know the rest.
 *
 * `composes` holds the qualified name of every kind this one composes,
 * ITSELF INCLUDED, so that `role into: sprout.Container` admitting
 * anything that composes `sprout.Container` is one lookup rather than a
 * walk. B19 computes the closure; asking the question is total.
 */
...
/**
 * Whether one kind composes another, which is the only way a kind
 * matches. A kind composes itself, so `Vessel` fills `role pot: Vessel`.
 */
...
/**
 * A type a value can actually have. The object type is in `SproutType`
 * because the spec's table of types names it, but it is never written
 * and no value is one, so the value arm of a binding excludes it rather
 * than leaving an unreachable case for every reader to rule out.
 *
 * Called `HeldType` and not `ValueType` for the reason `SproutType` is
 * not called that either: the previous language's extension interface
 * still exports the name, and B45 is the issue that takes it back.
 */
export type HeldType = Exclude<SproutType, { readonly type: 'object' }>;

/** A `SproutType` as a value type, or null where it is the object type. */
export function asHeldType(type: SproutType): HeldType | null {
...
export type BindingType =
  | { readonly binds: 'value'; readonly type: HeldType }
...
export function valueOf(type: HeldType): BindingType {
 ...
 */
export function forElementBinding(name: string, element: HeldType, at: Span): Binding {
...
export function handlerParameters(
  ...
  const carries = message.carries === null ? null : asHeldType(message.carries);
  if (carries === null) {
    diagnostics.refuse(
      value.at,
...
export function wasBinding(
  ...
  const held = asHeldType(property.type);
  if (held === null) {
    diagnostics.refuse(
      at,
      `\`:${property.name}\` holds no value a hook could carry.`,
      'A hook binds the value the property held before the change.',
    );
    return null;
  }
  return bind(name, valueOf(held), 'parameter', at);
}
