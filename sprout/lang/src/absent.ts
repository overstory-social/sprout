// What absent means (B04; the spec's The compiler › Strict and lenient,
// What absent means).
//
...
// than remember its own. Each row names the item that makes it real.
 ...
 * fatal. Named for the spec's own sentence — "the world runs with a
 * visible gap" — because the previous language's compiler still holds
 * `Absent` until the issue that replaces it deletes it.
 */
