...
import { defaultName, objectWords } from './names.js';
...
describe('an object in prose is its article and its name, or “you” to itself', () => {
  ...
  it('names anything else by its identifier humanised, after `a`, written “an” before a vowel', () => {
    const turn = proseTurn();
    ...
    expect(objectWords(CRATE, turn.marta, turn.context)).toBe('a crate');
  });
...
describe('a default name', () => {
  it('is an identifier with spaces for its underscores, or a kind’s name in lower-case words', () => {
    expect(defaultName('oak_door')).toBe('oak door');
    expect(defaultName('press')).toBe('press');
    expect(defaultName('PrintedSheet')).toBe('printed sheet');
    expect(defaultName('TVSet')).toBe('tv set');
    expect(defaultName('Key')).toBe('key');
  });
});
