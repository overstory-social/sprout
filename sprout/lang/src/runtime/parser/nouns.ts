...
// alike, when no answer could tell them apart and the nearest is taken.
...
/** One thing a noun may name, with how it is addressed. */
...
export function nounIn(
  ...
  if (pool.length === 1 || written.size === 1) return { found: 'one', id: pool[0]!.instance.id };
...
export function runIn(
  ...
  for (const { start, end } of nounsOfRun(words)) {
    ...
    const found = nounIn(words.slice(start, end), role, candidates, budget);
