...
import type { PropertyDeclaration } from './ast.js';
...
const ENUMS = table();
const WARD: SproutType = { type: 'symbol', of: ENUMS.qualified('sprout', 'Ward')! };
const DRYING: SproutType = { type: 'symbol', of: ENUMS.qualified('printers_shop', 'Drying')! };
...
describe('two types are the same one, or they are not', () => {
  it('matches a type with itself', () => {
    for (const type of [BOOLEAN, STRING, OBJECT, integer(), WARD]) {
...
describe('a type says what it is, in words a message uses', () => {
  it('names itself plainly', () => {
    ...
    expect(showType(OBJECT)).toBe('an object');
  });
});
...
describe('a literal is a value of a type, or it is refused at the literal', () => {
  ...
  const check = (type: SproutType, text: string) => {
    const diagnostics = new Diagnostics();
  ...
  it('refuses anything at all for the object type, which is never written', () => {
    expect(check(OBJECT, ':a 4').ok).toBe(false);
  });
});
