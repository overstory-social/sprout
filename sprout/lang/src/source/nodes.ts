// The rule every AST node keeps (the spec's The compiler › Diagnostics),
// and the walk that proves it.
//
// The spec asks for position spans on "every node". A node with no `at`,
// or one handed its parent's span, reports at the head of the definition
// and the author is back to reading a hundred problems at line 1. So the
...
//
// `unspanned` walks a tree and names every node that breaks the rule, by
// the path it sits at. Each suite that builds a tree runs it, so a node
// without a span fails the suite that built it rather than surfacing as
// a bad diagnostic later.
