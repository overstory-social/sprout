// Where a thing was written (the spec's The compiler › Diagnostics).
//
// "Every problem names the line and column of the thing it is about. Not
// the head of the definition — the token." A diagnostic can only name a
// token if the node it is complaining about remembered which token it
// came from, so every node the compiler builds carries a SPAN: two
// offsets into one file. Offsets rather than line/column pairs, because
// a span is carried by every node and computed from by almost none of
// them; the line index is built only when something is being reported.
