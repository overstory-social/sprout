...
// its own golden, and a changed line is a changed behaviour.
...
/** What the line above said, as the transcript writes it: indented, and `(nothing)` where nothing was. */
function indented(lines: readonly string[]): string[] {
  return (lines.length === 0 ? ['(nothing)'] : lines).map((line) => `  ${line}`);
}

/** `effects`, one line to each paragraph each reader read, under the reader's nickname and the kind. */
function effectLines(stage: Stage, effects: readonly Effect[]): string[] {
  return effects.flatMap((effect) => {
    const reader = stage.state.visitors.get(effect.visit)?.nickname ?? effect.visit;
    return effect.paragraphs.map((paragraph) => `${reader} (${effect.kind}): ${paragraph}`);
  });
}

/** A fault as the host would log it, against the object it names. */
function faultLine(stage: Stage, what: string, fault: Fault): string {
  const against =
    fault.object === null ? '' : `, against ${pathOf(stage.state.world, fault.object)}`;
  return `${what} faulted${against}, ${fault.name}: ${fault.detail}`;
}
...
function arrive(stage: Stage, nickname: string): string[] {
  ...
  const out = [
    ...
    ...caught.value.abandoned.map(
      (wake) => `left for live time: ${pathOf(stage.state.world, wake.object)}'s wake`,
    ),
  ];
  ...
  if ('closed' in arrived) return [...out, `closed: ${arrived.closed.words}`];
  if ('refused' in arrived) return [...out, ...effectLines(stage, arrived.effects)];
  return [...out, `not admitted: ${arrived.words}`, faultLine(stage, 'the arrival', arrived.fault)];
}

/** `@leave Marta`: a departure turn. */
function leave(stage: Stage, nickname: string, where: string): string[] {
...
function command(stage: Stage, nickname: string, text: string, where: string): string[] {
  const visit = present(stage, nickname, where);
  ...
  if ('choices' in turn.value && turn.value.choices.length > 0) {
    out.push(`choices: ${turn.value.choices.map((choice) => choice.line).join(' | ')}`);
  }
  return out;
}

/** `@tick`: one tick turn for each place a visitor stands in, in the host's order. */
function tick(stage: Stage): string[] {
  const out: string[] = [];
 ...
 */
function advance(stage: Stage, seconds: number): string[] {
  ...
  while (occupiedPlaces(stage.state).length > 0) {
    ...
    stage.state = turn.state;
    out.push(`${woken} woke, ${turn.value.elapsed} seconds after it asked`);
    out.push(...effectLines(stage, turn.effects));
  }
  stage.now = until;
  return out;
 ...
 */
function hostLine(stage: Stage, line: string, where: string): string[] | null {
...
export function playScript(bundle: Bundle, script: string, name = 'the script'): Played {
  ...
  for (const [i, line] of lines.entries()) {
    ...
    if (trimmed.startsWith('@')) {
      const made = hostLine(stage, trimmed, where);
      if (made !== null) page.push(...indented(made));
      continue;
    }
    ...
    page.push(...indented(command(stage, typed[1]!.trim(), typed[2]!, where)));
  }
