...
libraries, passages and the `.prose` files they live in) and checks the
expressions Phase 1 defined and the prose every passage holds; the runtime
reads a visitor's typed line as a reading, `go` among them, runs it as a
turn, and renders what is said and told once for each person who reads it.
Everything else — descriptions, the view — lands one backlog item at a time;
...
sprout/lang/src
  ...
  prose/     what is said, rendered for each reader: names, slots, blocks and loops, reflow, who hears it
