...
| `kinds.sprout`     | kinds and inheritance (`Cup: Vessel`, `Lantern: Usable`), an **abstract** message, every property type (`boolean`, `integer min/max`, `one_of … default`, a `string`, a `media` value), `:remembers`, `describe` with an `if / else if / else` chain, messages with **arguments**, several **`grammar`** lines and a **`when`** guard, `adjust`, **hooks** (`changed :lit`) that `broadcast`, **handlers** (`on :gust`, `on :fired`, `on :spawned`, `on :used` → `destroy self`), a container kind with **`pass`** rules and a **`release`** guard; a **`depart`** guard on `Vessel` (a raw pot refuses to be lifted) |
| `yard.sprout`      | `use media`; a room with `:image` and `show` in `describe`; `:remembers` on a room with `actor.remember` / `actor.recall`; exits; `on :entered` as arrival prose; a message on the room itself that `broadcast`s; an instance of a kind (`lantern: Lantern`) setting only initial values; `:scenery`                                                                                                                                                                                                                                                                                                                  |
