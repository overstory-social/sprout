...
- **83. What `actor` is typed as, now that an NPC acts.** Where types come from types `actor` by the world's visitor kind, and since 73 an NPC is not made of it: in a reading an NPC performs with `act`, a play that read what only the visitor kind has, `actor.get(:p)` or `act unlock (target: actor)` where only the visitor kind is `Lockable`, would read it off an NPC that lacks it, the unknown receiver the same section rules out. Built as proposed: `actor` is `sprout.Actor` in every body that binds it, so anything more is read through `is()`, `if (actor.is(Person)) { … }`, and a read without it is refused in the checker's existing words, "`sprout.Actor` has no `:oil`.", with a remedy naming the world's own kind that declares it, a person's first: "`:oil` is a `Person`'s. Read it as one first: `if (actor.is(Person)) { … actor.get(:oil) … }`." (`corpus/bad/actor-read-unnarrowed`). The remedy narrows any `get` so, a remembered property aside, and keeps the list of what the kind has where nothing of the world's declares the name. The spec's bindings table says the visitor kind; it needs to say `sprout.Actor`, or Eric chooses (c) the visitor kind in a person's reading and `sprout.Actor` in an NPC's, which the checker cannot tell apart, since any verb may be `act`ed.
- **84. A property the shared kind restates.** The worked microworld's `kind Visitor is Creature, sprout.Visitor { }` is refused: `Creature` restates `:capacity`, which makes it that property's origin ("Who a restatement's origin is", under B19), and `sprout.Visitor` brings `sprout.Actor`'s, so two origins reach the visitor kind. Built: the visitor kind restates it too, `kind Person is Creature, sprout.Visitor { :capacity 4 }`, as the corpus's `good/actors` does. Options: (a) as built, and the microworld restates; (b) a restatement by a kind that composes the property's origin supersedes that origin wherever both reach one composer, so a diamond through `sprout.Actor` resolves to `Creature`'s, and two restatements neither of which composes the other still collide.
- **85. A world's own `Visitor`.** The worked microworld declares `kind Visitor`, which hides `sprout.Visitor` and is warned under What it warns about ("`Visitor` hides `sprout.Visitor`: a bare `Visitor` in this world is now yours."). Built: warned; `sprout init`, every fixture and corpus world, and every remedy name the kind `Person` instead, so a fresh world checks with nothing said. Options: (a) as built, and the microworld renames its kind; (b) a kind that composes the library kind it hides, `kind Visitor is Creature, sprout.Visitor`, is not warned, since it plainly means both; (c) the library's kind takes a name a world will not want, such as `sprout.Person`.
- **86. Whether a later blessing reaches a world already published.** A load honours exactly the hashes blessed at publish, so a library the host blesses afterwards is still charged against a published world's caps until it is published again, and one un-blessed afterwards stays exempt. The spec says an un-blessing does not reach back; it is silent the other way. Built: neither reaches back. The alternative: a load honours the union of what was blessed at publish and what the host blesses now.
- **90. More than one `remembers` block in one body.** Per-actor memory says "a `remembers` block", and Declaring and composing lists "a `remembers` block" beside "one `describe`", so the spec does not say whether a second block is refused. Built: a body may write several, their entries combine as one, and a name in two of them is refused as a property declared twice ("`hall` holds `:a` twice."). The alternative: one block per body, a second refused at its word, as a second `describe` is.
- **91. Where a name in a kind's body resolves from.** A kind has no place in the tree, so Identifiers and scope's "nearest wins" cannot be asked of its body at compile time. Built: a kind's body names first what the kind itself gives each instance (`wick` in `Lantern`), then what the world's body holds, and the world's name as a first step reaches anything by its path; a nearer object around one particular instance is not what the name means. The alternative is to resolve from each instance's place at run time, which gives a spawned instance names no compile could check. (Recorded while building events; see below.)
- **92. What counts as an event.** Every event is charged against the turn's budget, and a move sends `:arrived` and `:departed` to everything in range of each place. Built: an event is a delivery that runs a handler or a hook; one to something with no handler for the message costs nothing. The alternative, every delivery counting, would let a crowded room's arrivals exhaust the default 256 before any handler ran. (Recorded while building events; see below.)
- **93. Whether the visitor kind's own body may write `without`.** Decision 80 lists what the visitor kind's body may declare and what it is refused, and `without` is in neither list. It declares no behaviour, but it leaves out a contribution the visitor kind composes, `without depart from Creature`, so what a person composes would not "run as it would for any object". Built: taken, as `contains` and kind contents are. The alternative: refused at the `without`, with the remedy of a shared kind that leaves it out, `kind Settled is Creature { without depart from Creature }`.
- **94. Whether `finally` is a reserved word.** `finally destroy self` makes `finally` a word of the syntax, and the spec's list of reserved words under Lexical rules does not include it. Built: read as a statement's first word, and not refused as a name. The narrow change is to add it to the list. (Recorded while building events; see below.)
- **95. How a kind's file is named.** The world model › Objects gives one example, `kind Chest` in `chest.sprout`, and says nothing of case or of a name of several words. Built, narrowly: the file is the kind's name in lower case with `_` between its words, `kind PrintedSheet` in `printed_sheet.sprout` (a capital starts a word after a lower-case letter or a digit, and the last capital of a run starts one when a lower-case letter follows, so `TVSet` is `tv_set.sprout`); only the last part of a file's path is read, so a library's `sprout/actor.sprout` is `actor.sprout`; and the name is compared exactly, so `Chest.sprout` and `printedsheet.sprout` are refused with the name to use. What Objects does not say either is how the rule fits Strict and lenient: built as a publish refusal and a load warning that leaves the file standing, as a file that travelled unnamed is, so a misplaced kind is not also refused everywhere it is named. The alternatives: compare case-insensitively; join the words with nothing (`printedsheet.sprout`) or `-`; read a misplaced kind's file as broken, absent at load. (Recorded while giving each kind its file; see below.)
- **96. Whether the world may share a kind's file.** Objects lists enums, verbs and messages as what may sit beside a kind, and the world is not among them; the worked microworld keeps it in `world.sprout` with enums, verbs and messages and no kind. Built: the world in a file named for one of its kinds is refused at the world's name, "The world `shop` shares `chest.sprout` with `Chest`, and a kind's file holds no world.", and the file the world is in is not otherwise named, so `world.sprout` is a convention and not a rule. The alternatives: the world may sit beside a kind, as the three may; or the world's file is always `world.sprout`. (Recorded while giving each kind its file; see below.)
- **97. Whether takeout carries what objects remember about a visitor.** Moderation and takedown says what objects remember about a visitor "is opaque to that visitor" and is erased when they are deleted; it does not say whether an export for that person includes it. Built: the store's `exportVisitor` carries it, each remembering instance's entry by its id, since an export is the host's to hand on or withhold and the store cannot know which. The alternative: the export carries the visitor record and instance only, and memory stays opaque even to takeout. (Recorded while reshaping core's records; see below.)
- **98. What forgetting a visitor does with what they held.** The spec erases memory "with their instance" and says nothing of the instance's contents. Built: removed with it, all the way down, as a destroy removes what a destroyed object held, and a declared object among them leaves no tombstone, so the next load makes it again where it was declared, at its defaults; a visitor forgotten while present is removed the same way, and nobody in range is told, since housekeeping is not a turn. The alternatives: (b) left inside an instance no longer stored, unreachable for good; (c) set down in the visitor's last place, which is a move the host may not make on the world's behalf. (Recorded while reshaping core's records; see below.)
- **100. What a thing's article is where none is written.** Addressing and display gives the default as "`a`; `the` for a unique fixture, `none` for a proper name", and no rule says what makes a fixture unique or a name proper. Built: `a` wherever nothing in the closure writes one, so `the` and `none` are what an author writes (`article the`), and a visitor, whose name is their nickname, is written with none. The alternatives: `the` for anything composing `sprout.Fixture`, or for the only instance of its kind in the world; `none` for a name written with a capital. (Recorded while building the command parser; see below.)
- **101. Two things a visitor cannot tell apart.** Disambiguation asks `which` among the candidates, and a `which` between two things written alike, two spawned coins both "a coin", has no answer the visitor could type. Built: where every candidate left is written with the same article and name, the nearest is taken and nothing is asked. The alternatives: ask anyway, and let the visitor fail to answer; or answer `unknown`.
- **102. When `unreachable` is said.** The world's `unreachable` passage says "You cannot reach {thing} from here", with `thing` bound, and nothing says what yields it. Range settles what it may not do: a command's nouns resolve against the visitor's range, a key in a shut chest "cannot be named until the chest is open", and places are out of range of one another, so naming something out of range would tell the visitor what a lid or a wall hides. Built: nothing out of range is named; a noun nothing in range answers to is `unknown`, so "take coin" beside the shut chest and "take barrel" in another place both say "That is not something you can do here." The passage and the answer are kept, and nothing yields `unreachable` yet, since no case in the spec names a thing the visitor can name that the verb cannot reach. The question for Eric: whether anything should, and what: a thing in range that a role's requirement puts out of reach (a verb that wants what the actor holds, or what is on a surface rather than in it), a thing reached only as a surface, or nothing, and the passage is dropped from the library.
- **103. `through` in the spec's own exits.** Exits gives the directions as a closed set, `north`, `south`, `east`, `west`, their diagonals, `up`, `down`, `in` and `out`, and Places inside places writes `exit through "through the fur coats" -> narnia`. Built: the closed set, with the diagonals written as one word (`northeast`, `ne`), and `through` is no direction; B28 declares exits and meets it first. The alternatives: add `through` to the set; or read the example's direction as a label only.
- **104. Which verb a phrase reaches when a world's verb shadows a library's.** A world's own `take` shadows `sprout.take` with a warning (Libraries and namespaces), and B23 left whether both are offered to the parser. Built: the world's own verb's phrases are typed and the shadowed library verb's are not typed at all, so `pick up the key` reaches nothing where the world's `take` does not write it. The alternative: the library verb's phrases stay typed where the world's verb writes none that match.
- **110. Whether the engine's own defect is a fault.** Faults says a command turn that faulted is abandoned and its actor told through the world's `fault`, and does not say whether a defect of the engine's, something no rule of the language explains, is one. Built: every throw from a turn's body is a fault, abandoning the transaction and told the same way, since no turn ends with nothing said; one that is no rule of the language's is marked `engine` for the host to report loudly, as an actor found where actors cannot stand is. The alternative: an engine defect is not the world's fault, and the turn ends at the host with no world text, which leaves the visitor with nothing. (Recorded while building turns; see below.)
- **111. What a view's "world version" is.** The view says a view is valid until the world's next write turn, so a host may cache it by world version and visitor, and nothing the engine stores is that version: the serial moves only when an id, an arrival or a wake is issued, so a turn that only sets a property leaves it where it was. Built: a committed write turn names every visitor present, whose views it made stale, and a faulted one names none, since the world is exactly as it was; no version is stored. The alternatives: (a) a counter every committed write turn advances, stored with the serial, which a host caches by; (b) the log's position (B40), which is the same counter kept elsewhere. Either is B37's to build. (Recorded while building turns; see below.)
...
- **Whose kind may act.** The kind that wrote the body must compose `sprout.Actor`, as What it refuses words it, so a behaviour kind meant for an NPC composes `sprout.Actor` itself, `kind Purring is sprout.Actor { … }`, and composes beside `Creature` (where `Creature` restates a property of `sprout.Actor`, Open 84 applies). The refusal reads "Only an actor acts, and `Kettle` does not compose `sprout.Actor`.", the words a play `as actor for` on such a kind is already told.
...
- **The name a remedy suggests.** `Person`, and `Guest` where the kind written is already `Person`, since a world's own `Visitor` is warned (Open 85).
...
- **What `actor` is.** `sprout.Actor`, in every body that binds it (Open 83); no corpus world or fixture read a person's own property through `actor`, so none needed narrowing.
...
- **What else the body may say.** `contains` and kind contents (objects in its body) are what a person holds, and are taken; so is `without`, which Open 93 asks about.
...
- **Several blocks in one body.** Allowed, their entries combining (Open 90).
...
- **What an event is.** One delivery that runs at least one handler or hook, charged once however many composed handlers it runs; a delivery to something with no handler for the message costs nothing, since a move's `:arrived` goes to everything in range of the place. A real question for Eric: whether every delivery counts, handled or not, which would let a busy room's arrivals exhaust the budget. Open 92.
...
- **`finally destroy self`.** Built here, since it needs the drain: `finally` then `destroy self` is one statement, standing wherever `destroy self` may, refused with "Only `destroy self` may follow `finally`." otherwise. It marks `self`; once the queue is empty each marked object still there is destroyed in the order marked, as `destroy self` would, and that sends nothing. Written beside `destroy self`, the destroy happens at the body's end and the mark comes to nothing. The warning for destroying a declared object covers it. `finally` is not among the reserved words the spec lists; it is read as a statement's first word, and nothing refuses it as a name. A real question for Eric: whether `finally` joins the reserved words. Open 94.
...
- **The name.** A kind's name in lower case, `_` between its words, compared exactly against the last part of the file's path (Open 95).
...
- **The world.** Refused in a kind's file, at the world's name, with "Move the world to a file with no kind in it, such as `world.sprout`." (Open 96). A world in a file with kinds none of which it is named for is not refused: the kinds are, and moving them leaves the world where it is.
...
- **What forgetting takes.** The visitor record, their instance, everything stored inside it all the way down, and every instance's memory keyed by their instance id, dormant records included; their own instance's memory of others goes with it. The serial and the tombstones are untouched. What they held is Open 98.
- **What exporting gives.** Per world: the visitor record, their instance (null where the store lacks one), and each instance's memory of them, by the remembering instance's id; not what they held, which is the world's. Whether memory belongs in it is Open 97.
...
- **What a write turn is given.** The seed the host drew, which B33 draws from, and the host's bound on stored instances for the turn, both for the log to record (B40); the host's budgets; and the host's clock, which only the wall-clock backstop reads. `elapsed` is a tick's and a wake's, and B35 and B36 give it.
...
- **Stale views.** Every committed write turn names every visitor whose instance stands somewhere, in code-unit order of the visit, as one whose view is stale, a refused command and a parser's answer included, since each is a write turn; a faulted turn names none (Open 111).
...
- **The other kinds' faults.** The frame abandons a tick, a wake and a maintenance turn as it does a command, and returns the fault; what follows is each kind's own item: a faulted tick dropped (B35), a faulted wake consumed and logged (B36), which writes after the fault, and a faulted maintenance turn's catch-up abandoned with the visitor admitted (B36). A poll that faults yields the world's `unseen` speech; logging it against the object whose `describe` cost too much is B37's.
...
- **`unreachable`.** Open 102. Nouns resolve only against the visitor's range, walked once by the pass rules; nothing out of it is named, in a reading or in an answer, and a noun nothing in range answers to is `unknown`. Nothing yields `unreachable` yet.
...
- **What the answers carry.** The world's `unknown`, `unreachable` and `which` passages, unrendered, with `actor` and `here` bound, and `thing` or `candidates` as the bindings table says; B29 renders them. A world that composes none of them is an engine error, as for `nothing_happens`.
