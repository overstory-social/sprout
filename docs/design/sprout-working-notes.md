...
- **130. What the first tick a place receives hands `elapsed`.** Ticks says `elapsed` is the seconds since the last tick the place actually received, and a place that has never been ticked has none. Built: 0, and the tick records when it ran, so the second tick carries the first real interval. The alternatives: (a) the seconds since the place was first occupied, which needs an instant the engine does not store; (b) the seconds since the world was loaded or published, which is the host's and not the place's. (Recorded while building ticks; see below.)
- **131. Whether `elapsed` spans a stretch nobody was there.** Empty places are not ticked, and the spec's words, "the seconds since the last tick this place actually received", make the first tick after a day's absence carry a day, so a moor counting seconds to its next gust gusts at once. Built to the letter. The alternative: a place's last tick is forgotten when its last visitor leaves, so ambience resumes as if fresh, which reads 130's answer again on every return; the spec's framing of a tick as ambience, meaningful only when perceived, argues for it, and its "skipping is invisible to an author who accumulates it" argues against. (Recorded while building ticks; see below.)
...
- **`elapsed`.** The seconds from the place's last tick to this one's instant, which the turn then records as the place's last tick. A place's first tick is handed 0 (Open 130). An empty stretch is not forgotten: the next tick spans it (Open 131).
