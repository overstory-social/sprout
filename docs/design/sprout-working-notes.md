...
- **Whose kind may act.** The kind that wrote the body must compose `sprout.Actor`, as What it refuses words it, so a behaviour kind meant for an NPC composes `sprout.Actor` itself, `kind Purring is sprout.Actor { … }`, and composes beside `Creature` (where `Creature` restates a property of `sprout.Actor`, Open 82 applies). The refusal reads "Only an actor acts, and `Kettle` does not compose `sprout.Actor`.", the words a play `as actor for` on such a kind is already told.
...
- **The name a remedy suggests.** `Person`, and `Guest` where the kind written is already `Person`, since a world's own `Visitor` is warned (Open 83).
