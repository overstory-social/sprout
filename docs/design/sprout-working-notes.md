...
- **Where an NPC may be declared.** An NPC needs no place among its ancestors, and an actor may be declared inside something that does not hold actors (a cat in a basket); both are accepted, since where an actor may be moved is B22's and B42's. Decided 2026-09-23. *Decided 2026-09-23 (43, 62), the other way:* an actor declared inside something that does not hold actors is refused; the code catches up.
...
- **An actor that is not an NPC.** Whether an object composing `sprout.Actor` but not the visitor kind is an actor that cannot `act` is open; `isNpc` answers only whether an object composes the visitor kind. Eric leans to yes and asked for more on the question, 2026-09-23. *Decided 2026-09-23 (21):* there is none; such an object is refused. The code catches up.
...
- **What a destroyed object held.** Falls to its container without consent, as Destroying says, each thing placed last in that container in the order it was held, so the fallen keep their order after whatever was already there. Decided 2026-09-23 as the spec is written, until Eric confirms a change: his preference, pending, is that what it held is destroyed with it. *Decided 2026-09-23 (26), the other way:* destroyed with it, all the way down; the code catches up.
- **The `:entered` for what fell.** One per thing, in falling order, `from` the destroyed object. The destroy returns them and the queue drops them under the no-effects rule above, since their `from` is gone, so the container is never told. Destroying still says the container receives `:entered` for each thing that fell, which that rule contradicts; the contradiction is recorded for Eric, and the rule lives in one place, the queue's, whichever way it goes. *Moot, 2026-09-23 (55):* nothing falls.
- **A visitor standing directly in a destroyed object.** A fault, whatever the object is. Decided 2026-09-23. A visitor further in — in a box in the room — moves with what holds them. *Decided 2026-09-23 (27):* anywhere inside, now that contents are destroyed; the code catches up.
...
- **Dormant contents of a destroyed object.** Left where they are: a dormant record is kept untouched, so it still names the destroyed object as its container. When its kind returns it decodes into a container that no longer exists and is not live; where it goes then is open. *Decided 2026-09-23 (26):* destroyed with it; the code catches up.
...
- **`here`.** The actor's nearest container that holds actors, so a wardrobe declaring `contains actors` is `here` for whoever stands in it. An actor with no such container, as an NPC in a crate in a world that holds only things, has the world as `here`; the spec does not say. *Moot, 2026-09-23 (62):* every actor has a place, so the world fallback never applies; the code catches up.
