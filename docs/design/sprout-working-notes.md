...
*Closed on 2026-09-21, by Eric, against B03:* **what a manifest holds.** It carries the world's name, its version, its author, its licence, the version and content hash of every library it vendored, and an enumeration of its own `.sprout` and `.prose` files. Two more are the spec's own and stay: the language level (`a microworld's manifest records the level it was written for`) and the extensions it pins (`a world names the extensions it uses, pinned by major version, in its manifest`). The spec's Files section should say so; the change is proposed in the PR rather than made here.
