...
- **120. Where `$index` counts from.** Conditionals and loops binds `$index` and says only that it is an integer. Built: from 1, so `{if $index == $count}` is `$last`. The alternative: from 0. (Recorded while building passages; see below.)
- **121. The article `a` before a vowel.** Addressing and display gives `a` as the default article and lists `a`, `the` and `none`, not `an`, so "a oak door" is what the default would say. Built: `a` is written "an" before a name that starts with a vowel letter, until B27's grammar block gives an author the word. The alternatives: an `an` article an author writes, with `a` always "a"; or a rule on sound rather than letter. (Recorded while building passages; see below.)
- **122. What the engine binds when it says its own lines.** Where types come from names `thing` in `unreachable` and `unremarkable` and `candidates` in `which`; `inside_itself`, `arrives` and `leaves` render `{item}`. Built: each engine line binds exactly those, and nothing else, not even `actor` and `here` for a line said to the person acting, such as `nothing_happens` or `unknown`, so a world's `passage nothing_happens { {actor} stands there. }` is refused. The alternative: the lines said to a person acting (`unknown`, `unreachable`, `which`, `nothing_happens`, `unremarkable`, `inside_itself`) bind `actor` and `here` as `describe` does. (Recorded while building passages; see below.)
- **123. What a passage a slot renders is given.** Slots says `{pot.greeting}` runs "with that object as its own `self`", and Passages that `actor`, where bound, is the visitor in any passage those bodies invoke. Built: the passage is given `actor` and `here` where the slot has them, and nothing else: no role, no `let`, no loop variable. The alternative: everything in reach where the slot is, as a passage said by name is given. (Recorded while building passages; see below.)
- **124. A `.prose` file for two kinds, and two files for one.** Passages says long passages live "in a file the kind points at". Built: one `prose` line per body, and one kind per file; a second line is refused at it, and a file a second kind points at is refused at the second. The alternatives: a kind may point at several files, and several kinds (a family of kinds, or an object and its kind) at one. (Recorded while building passages; see below.)
...
- **126. `here` in a `{for}`.** Where types come from types `here` as an object, so `{for thing in here}` is refused until narrowed, as `here.count` is: "Sprout does not know whether this holds anything." The actor's place always holds things. The alternative: `here` is typed as something that holds things, so a description can walk it without an `is()`. (Recorded while building passages; see below.)
- **127. The literal cap and `refuse`.** Static caps caps "a `say`, `tell` or `text` written as a literal"; `refuse` is not among them. Built: `say` in quotes is held to it (B30 adds `tell` and `text`), and `refuse` in quotes is not. The alternative: every line in quotes, `refuse` included. (Recorded while building passages; see below.)
...
- **`{item}` in `arrives` and `leaves`.** Untyped by the binding table. *Built (B29):* the engine says a place's notices with `item` bound to what moved, an object, and nothing else (Open 122).
...
- **What an object is called.** Until B27's grammar block, an object is `a` and its identifier humanised, a spawned instance or a kind's content its kind's name humanised (`PrintedSheet` as "printed sheet"), and a visitor their nickname with no article; "an" before a vowel (Open 121). To its own reader any object is "you", and the verb's agreement is the author's.
- **Words in quotes.** A one-line passage: a `}` alone is a character, since nothing in quotes can escape one, and a `{` never closed is refused at it. `say` in quotes is held to the host's literal cap, counted after escapes (Open 127).
...
- **The engine's fixed words.** An actor moved into what holds no actors reads a one-line passage, `{item} cannot stand in {to}.`, rendered with the actor and where it was to go, so the words name them as any slot does.
