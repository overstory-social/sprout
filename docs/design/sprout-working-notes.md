...
- **Where a part's language level comes from.** A bundle's level is the highest of any of its parts, and a part's level is raised by the syntax it uses — which nothing can derive until there is a parser that knows which syntax raised it. A library declares its level alongside its source for now, and the world declares its in the manifest.

The bundle carries three things the spec's list does not name: the manifest it was compiled from, which is where the world's name comes from and so the namespace its declarations are unqualified in; the size its source came to, so that what was counted against a cap and what was exempt is legible rather than re-derived; and the bundle's own hash, which the log already records beside every publish.

*Closed on 2026-09-21, by Eric, against B03:* **what a manifest holds.** It carries the world's name, its version, its author, its licence, the version and content hash of every library it vendored, and an enumeration of its own `.sprout` and `.prose` files. Two more are already there and stay. The extensions it pins are the spec's own: “a world names the extensions it uses, pinned by major version, in its manifest”. The language level is not — the spec says only that a bundle's level is the highest of any of its parts, and where a part declares its own is the open question two entries above. The manifest is where the world declares its, for now, which is a decision and not a reading. The spec's Files section should say what a manifest holds; the change is proposed in the PR rather than made here.
