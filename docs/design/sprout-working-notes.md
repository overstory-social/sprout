...
- **125. A passage nothing says.** A passage is checked against every body that says it; one that nothing says, and the engine does not, is checked with its own `self` alone, so `passage stare { {actor} stares. }` that no body says is refused as "Nothing here is called `actor`." The alternative: checked only for what does not depend on who says it, and left to the warning What it warns about lists for a passage nothing invokes. (Recorded while building passages; see below.)
...
- **133. A `:tick` handler on something that is never a place.** The worked microworld's cat, whose first draft handled `:tick`, "would never have run", and What it warns about does not list it. Built: accepted and never delivered, with nothing said, since a kind may be composed into a place elsewhere and only an object's whole kind says whether it holds actors. The alternative: warned at an object, declared or spawnable, whose kind answers `:tick` and does not declare `contains actors`, "`cat` is not a place, so `on :tick` never runs. Send it something from its place's tick instead." (Recorded while building ticks; see below.)
- **143. A wake on something out of the tree.** A wake is on an object, and an object inside an away visitor, or inside something of an absent kind, is not in the tree. Built: such a wake is not due until the object is back, and is then delivered with an `elapsed` that spans the whole time, so a candle carried away still goes out; catch-up before the visitor's own admission does not reach what they carry, so it is delivered live once they are in, and narrates. The alternative: it is due as any other, and the queue drops its `:woke` because the object is not live, so it is consumed with nothing done. (Recorded while building wakes; see below.)
- **144. What catch-up drops.** Absence says a wake delivered after an absence applies its state changes "and its `tell` is dropped". Built: a maintenance turn gives back which wakes it delivered and nothing any of them said — a refused `move`, which is said to nobody, and an NPC's reading performed with `act`, whose `say` goes to that NPC, included — so B30's `tell` needs no rule of its own there. The alternative: only `tell` dropped, and what an `act` says kept, which reaches a person only where a kind a visitor composes performs it. (Recorded while building wakes; see below.)
- **145. A wake nobody answers, and a `:woke` nobody asks for.** What it warns about lists neither. Built: a `wake` on a kind that answers no `:woke` is asked for, waits and is consumed with nothing done, counting against the cap meanwhile; an `on :woke` on a kind whose bodies never write `wake` is accepted and runs only if something composed with it asks. The alternative: warn at each, as a handler nothing sends to is warned about, "Nothing here asks to be woken, so `on :woke` never runs.", with the same caution as 133 that composition may bring the other half. (Recorded while building wakes; see below.)
- **180. What the checker knows of a name in a kind's body.** Decided 91: a name in a kind's body resolves at run time from where each instance sits, nearest wins, so a compile can no longer say which object it names, type its reads at that object's kind, or refuse it for reaching nothing, since a spawned instance may sit anywhere. What the checker holds such a name to, and what one that reaches nothing when it runs does, is the implementing issue's to propose, not built. The narrow reading: a name no object in the bundle declares is refused as now; a read through it is of the object type and needs `is()`, as a handler's sender does; one that reaches nothing when it runs faults as a name out of range does (`NameOutOfRange`), and a `send` to it does nothing. (Recorded while folding in 91.)
- **181. Whether `unreachable` leaves the standard library.** Decided 102 answers a noun nothing in range answers to with `not_here`, "You see nothing like that here.", and names no case for `unreachable`. Read narrowly: `not_here` replaces it, and the spec's standard library and bindings table no longer carry `unreachable`. The alternative: keep it for a thing in range that a role's requirement puts out of reach (a verb that wants what the actor holds), which nothing in the spec names yet. (Recorded while folding in 102.)
...
- 82: the engine reads a destroyed declared place as it reads an absent one: an exit to it does not apply, and a world whose arrival place is destroyed admits no one (Destroying). Not reached yet; exits (B28) and admission (B42) build it.
...
- 101: among things written alike, the nearest is meant, and among equally near ones a draw from the turn's seed, "so people don't depend on unintentional tie breaks" (Spawning, which said the first in its container's order). Built differently: the draw; issue to follow.
- 102: "only what the player names": a noun nothing in range answers to is answered with a new stock line on `sprout.World`, `passage not_here default { You see nothing like that here. }`, bound `actor` and `here` as the parser's other answers are (Range, the bindings table, the standard library). `unreachable` is read as replaced and leaves the standard library (Open 181). Built differently; issue to follow.
- 103: the example was wrong: Places inside places now writes `exit in "through the fur coats" -> narnia`, and the closed set stands, as built. Every other `exit` in the spec is in the set; the Links example's `link onward` and `link back` are not either, and are Open 150's, left as they are.
...
- 121: the default `a` is written `an` before a name beginning with a, e, i, o or u; a written `article` still decides (Addressing and display). Built differently; issue to follow.
...
- 126: `here` is typed `sprout.Place` (the bindings table). Built differently; issue to follow. A place is any object declaring `contains actors` (Places), so an actor's place need not compose `sprout.Place`; the issue says how `here` is typed there.
- 127: the literal cap holds `refuse` in quotes too (Static caps). Built differently; issue to follow.
...
- **A world whose composed kinds lack an engine passage.** A forked standard library without `fault` or `displaced`: what the engine says then is B34's and B48's. *Built (B34), for `fault` and `unseen`:* the stock line in fixed words.
...
- **What a `do` does at this level.** `let`, `if`, `spawn`, `destroy self`, the four writes, `remember` and `say`. `tell`, `move`, `send` and `act` are not yet statements the parser reads; B30, B25, B32 and B26 bring them. *Built (B32):* `send` and `broadcast` are statements a `do`, a handler and a hook hold.
...
- **`say` in a handler or a hook.** Refused in the spec's own words from Diagnostics, whose remedy names `tell`, which B30 brings; until then the remedy names a statement this compiler does not read yet.
...
- **Identifiers in an expression.** One name only, `lamp.get(:lit)`; a dotted path is written where a thing goes, after `send`, `move`, `spawn … in` and a role of `act`, since `.` in an expression reads a member. An identifier is typed at its object's own kind, so its properties read without `is()`.
- **Where a copy is found at run time.** For a declared instance, at its declared path under the instance; for a spawned one, the first instance inside it, breadth-first, made as that kind's copy at that path. A copy moved out of the instance that was given it is no longer found, and the name reaches nothing.
...
- **`{one of}`.** Chance's (B33): refused as not read, and the choice stepped over to its `{/one of}`.
...
- **What a command turn gives back.** Its phases as they are, not one list: the parser's answer; the consent pass's refusal; or what the effect pass did and what the queue did after. Ordering them into one sequence of effects is B38's.
...
- **The other kinds' faults.** The frame abandons a tick, a wake and a maintenance turn as it does a command, and returns the fault; what follows is each kind's own item: a faulted tick dropped (B35, built: see "Found while building ticks"), a faulted wake consumed and logged (B36), which writes after the fault, and a faulted maintenance turn's catch-up abandoned with the visitor admitted (B36). *Built (B36):* a faulted wake's turn is abandoned and the wake then consumed in a turn of its own, and a maintenance turn keeps what came before a faulted part (Open 140); see "Found while building wakes". A poll that faults yields the world's `unseen` speech; logging it against the object whose `describe` cost too much is B37's. *Decided 2026-09-24 (140), the other way:* the other objects' due wakes are still delivered; the code catches up.
...
- **`tell` in a tick.** Not yet read (B30); what a tick says today is what its drain says, a refused `move` to nobody and an NPC's reading performed with `act`.
- **A `:tick` handler on something that is never a place.** Accepted, and never delivered (Open 133).

Found while building the command parser (B27), each decided the narrow way and awaiting Eric:

- **What a grammar block holds.** `name` and one name in quotes, `article` and one of `a`, `an`, `the` or `none`, and `nouns` and one or more nouns in quotes, in any order. `exit` and `link` are B28's, and until then a line the block is not made of is refused once, the rest of that line stepped over (`corpus/bad/grammar-lines`). The world's own body reads no grammar block, and the world is never a noun's candidate, though range reaches it as a surface: nothing addresses the world.
...
- **`unreachable`.** Open 102. Nouns resolve only against the visitor's range, walked once by the pass rules; nothing out of it is named, in a reading or in an answer, and a noun nothing in range answers to is `unknown`. Nothing yields `unreachable` yet. *Decided 2026-09-24 (102), the other way:* a noun nothing in range answers to is answered with `not_here`; the code catches up (Open 181).
...
- **The word set.** Single words, sorted: each word of every name and noun written, of every object's identifier at every depth, and of the humanised name of every kind a spawn may name that writes no name, beside the directions, their abbreviations, the determiners, `and` and every phrase's words. A kind nothing spawns, the world's and the visitor kind, gives none. Exit labels join it with B28.
- **What the answers carry.** The world's `unknown`, `unreachable` and `which` passages, unrendered, with `actor` and `here` bound, and `thing` or `candidates` as the bindings table says; B29 renders them. *Built (B29):* `renderFor` renders an answer for its reader. A world that composes none of them is an engine error, as for `nothing_happens`.
...
- **What is due, and in what order.** Every wake whose instant has come on an object in the tree, oldest first by when it fell due and then serial (Open 141); a wake on something out of the tree waits (Open 143). A destroyed object's wakes go with it, and with what it held. *Decided 2026-09-24 (141):* as built; 143 is still open.
...
- **Catch-up.** One maintenance turn, under one budget: each object's oldest due wake, one per object, oldest first, each a part kept once it has run; a wake one of them asks for waits for live time, as do an object's later due wakes. A part that faults is consumed and ends the catch-up, what came before it kept (Open 140). The turn always commits, if only an empty change set, and gives back what it delivered, the wake that faulted with its fault, and what it abandoned; nothing said (Open 144). Its change set is everything between the state it opened on and the state its last part left (`changesBetween`). *Decided 2026-09-24 (140), the other way:* a part that faults does not end the catch-up; the code catches up.
- **Admission.** Core's `runMaintenance` never refuses a world's fault, so the host admits the arriving visitor once it resolves; wiring it before entry is B42's.
- **The seed.** Every part of a maintenance turn is handed the turn's one seed; drawing from it (B33) must keep one stream across the parts, or the parts draw alike.
