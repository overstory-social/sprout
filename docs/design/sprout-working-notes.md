...
- **77. Whether the place itself and a surface are sent a place's message.** Built by the letter of After the move, "every other object in range": the walk from the place reaches the place itself, which is sent `:departed` or `:arrived` beside its `:left` or `:entered`, and the first container outward that refuses, as a surface, which for a place directly in the world is the world, sent one of each for every actor's move in every place. A broadcast leaves out both. Options: (a) as built; (b) leave out the place, which already hears `:left` and `:entered`; (c) leave out a surface, as a broadcast does; (d) both.
...
- **A place that writes no `leaves` or `arrives`.** Its visitors read nothing for that side, and its range is still sent the message, since the message is the engine's and not the passage's.
