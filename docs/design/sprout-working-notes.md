...
- **What a manifest holds.** The spec names the manifest as one of the four things a microworld is made of and never says what is in it, and the bundle it describes does not carry the world's own name. The manifest carries the world's name, the level it was written for, the extensions it pins and the libraries it uses, because each is needed before a line has parsed — naming the libraries in particular is what lets closedness be checked without a parser. B12's `world` declaration will have to agree with that name, or replace it.
