...
- **Whose kind must compose the visitor kind.** The kind that wrote the body, as What it refuses words it ("a body whose kind does not compose the visitor kind"), not the object running it. So a behaviour kind written to be composed into an NPC's kind, `kind Purring { as target for pet { do { act purr () } } }` with `kind Cat: Creature, Purring`, is refused, though every object running it could act. A real question for Eric: it decides whether reusable NPC behaviour can live in a kind of its own. Open 73. *Decided 2026-09-23 (73), the other way:* any composer of `sprout.Actor` may `act`. The code catches up.
