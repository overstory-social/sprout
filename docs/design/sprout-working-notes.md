...
- **What the drain is given and gives.** `drain` takes what the effect pass queued, destroyed and marked, and gives back what handlers said, the notices of any actor they moved between places, everything destroyed, and how many events ran. It is not a turn: B34 runs the consent pass, the effect pass, the drain and the views in order, under one transaction.
...
- **Set roles.** A comma before `and` is one split; an empty noun in a run, `juggle gong and`, makes the phrase not match. How many things one set role binds is checked against the host's budget only for the reading that wins, and past it is a fault, as Limits says of a budget.
