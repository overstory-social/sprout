...
- **A `without` in a kind that is itself composed.** Whether what `B` leaves out stays left out in `D: B`, and what happens when `D` also reaches the source another way, was unsaid. *Decided 2026-09-23:* it stays left out where `D` reaches the source through `B`, and a copy reaching `D` by another path still runs; the guards build it so.
