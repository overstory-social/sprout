...
- **76. The spec's leftovers of `in`.** Names › Identifiers and scope still reads an object's `in` from inside the world (the paragraph beginning "Nearest-wins is the rule for identifiers inside a body") and states the hiding rule twice; What absent means still has the row "a container, in an object's `in`". Built without either, as Objects says. To do: take both out of the spec.
...
- **The spec's leftovers of `in`.** Names › Identifiers and scope still has a paragraph reading an object's `in` from inside the world, and repeats the hiding paragraph; What absent means still has the row for a container in an object's `in`. Built as The world model › Objects and the decision above say: no `in`, no `container` row. Open 76.
