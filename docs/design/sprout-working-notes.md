...
- **102. What `unreachable` may name.** The world's `unreachable` passage says "You cannot reach {thing} from here", and nothing says which things out of range it may name. Built: a noun that names nothing in range but answers, with a thing that fits the role, to something anywhere in the world is `unreachable`, the nearest such thing by a walk of the whole tree; so "take coin" beside a shut chest says the coin cannot be reached, which tells the visitor what the chest holds, and "take barrel" names a barrel in another place. The alternatives: only what is inside something in range (the chest's coin, not the cellar's barrel); only what the actor's place holds; or never, every such noun `unknown`.
...
- **The order phrases are tried in.** The world's own verbs, then the standard library's, then every other library's, each in declared order; the first reading wins; failing one, the first `which` any phrase asked; failing that, `unreachable`; else `unknown`. The world's own verb shadows a library verb of its name (Open 104).
...
- **`unreachable`.** Open 102. The walk that finds it ignores pass rules and is charged to steps as a range walk is, and it is taken only when no phrase read or asked `which`.
